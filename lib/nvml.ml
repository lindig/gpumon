...
module NVML : sig
  ...
  let with_mutex mx f =
    Mutex.lock mx;
    Stdext.Pervasiveext.finally f (fun () -> Mutex.unlock mx)
  ...
  let open_nvml_interface () =
    ...
    try
      ...
      interface
    with e ->
      library_close interface;
      raise e

  let get () = !interface

  let attach () =
    with_mutex mx @@ fun () -> match !interface with
    | Some _ ->
      D.warn "Nvml library attach: library already attached"
    | None ->
      begin match open_nvml_interface () with
        | i ->
          interface := Some(i);
  ...
  let detach () =
    ...
    | Some intf  ->
      ...
      Stdext.Pervasiveext.finally
        (fun () -> shutdown intf)
        (fun () -> interface := None; library_close intf)
